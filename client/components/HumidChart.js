    ...
    >
      {slice.points.map(point => (
        ...
        >
          ...
          <p style={{ fontFamily: 'Inconsolata' }} className="pt-2 border-t border-gray-800">🌫️: <span className='-ml-2 font-semibold text-sm'>{point.data.yFormatted}%</span></p>
        </div>
      ))}
    </div>
  )
...
const HumidChart = ({ data, timeSpan }) => {
  ...
  return (
    <div className="rounded-lg shadow-inner bg-white pb-4 h-64 w-64 sm:w-72 sm:h-72 md:h-96 md:w-96 xl:h-120 xl:w-132">
      <p className="font-bold text-2xl text-center -mb-10 mt-2 underline">Air Humidity</p>
      <ResponsiveLine
        data={data}
        curve="basis"
        ...
        yScale={{ type: 'linear', min: "0", max: "80", stacked: false, reverse: false }}
        lineWidth="2"
        gridYValues={[0, 10, 20, 30, 40, 50, 60, 70, 80]}
        enableSlices="x"
        enablePoints={false}
        enableArea={true}
        areaOpacity={1}
